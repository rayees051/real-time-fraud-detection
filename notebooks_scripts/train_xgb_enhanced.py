# notebooks_scripts/train_xgb_enhanced.py
import os, joblib
...
SRC = "data/paysim_good.csv"   # your prepared dataset
...
# is_new_device (shifted so it reflects history)
df['is_new_device'] = 0
...
# --- NEW engineered features to amplify anomaly signals ---

# huge_amount_flag: 1 if amount >> customer's historical mean (e.g. > 5x)
df['huge_amount_flag'] = (df['amount'] > (df['cust_cum_mean'] * 5)).astype(int)

# new_city_flag: 1 if current city != previous city (based on shift)
if 'city' in df.columns:
    df['prev_city'] = df.groupby('customer_id')['city'].shift().fillna(df['city'])
    df['new_city_flag'] = (df['city'] != df['prev_city']).astype(int)
else:
    df['new_city_flag'] = 0

# log ratio + interaction
...
# risk_score: a simple weighted linear combination (helps model)
df['risk_score'] = (
    0.6 * df['log_amt_to_cum_mean'].fillna(0) +
    1.0 * df['is_new_device'].astype(int) +
    0.8 * df['new_city_flag'].astype(int) +
    0.4 * df['is_night'].astype(int) +
    0.5 * df['huge_amount_flag'].astype(int)
)

# Final feature list for training (must match runtime consumer)
features = [
    ...
    'huge_amount_flag','new_city_flag','risk_score'
]
...
# Use all frauds + sampled negatives (safe-guard)
...
# xgboost params with imbalance handling
